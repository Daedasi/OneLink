...
// <editor-fold defaultstate="collapsed" desc="Declaración de sentencias SQL">
    private static final String SQL_SELECT_EMPLEADOS = "SELECT Empleados.idEmpleado ID, Empleados.Nombre, Empleados.Apellido, tiposDocumento.idTiposDocumento, tiposDocumento.label TipoDocumento, Empleados.numDoc, Areas.idArea, Areas.label Area, SubAreas.idSubAreas, SubAreas.idAreas, SubAreas.label SubArea\n" +
                                                        "FROM Empleados\n" +
    ...
    private static final String SQL_SELECT_EMPLEADOSSEARCH = "SELECT Empleados.idEmpleado ID, Empleados.Nombre, Empleados.Apellido, tiposDocumento.idTiposDocumento, tiposDocumento.label TipoDocumento, Empleados.numDoc, Areas.idArea, Areas.label Area, SubAreas.idSubAreas, SubAreas.idAreas, SubAreas.label SubArea\n" +
                                                            "FROM Empleados\n" +
                                                            "LEFT JOIN tiposDocumento ON Empleados.idTipoDocumento=tiposDocumento.idTiposDocumento\n" +
                                                            "LEFT JOIN Areas ON Empleados.idArea=Areas.idAreas\n" +
...
//Listar EmpleadoDTO para visualización
    public List<EmpleadoDTO> listarEmpleados() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                empleado = new EmpleadoDTO(idEmpleado, nombre, apellido, tipoDoc, area, subArea);
    ...
    public List<EmpleadoDTO> buscarEmpleados(String numDocSearch, String nombreSearch) {
        Connection conn = null;
        PreparedStatement smt = null;
        ResultSet rs = null;
        EmpleadoDTO empleado;
        ...
        try {
            ...
            smt.setString(1, numDocSearch);
            smt.setString(2, nombreSearch);
            rs = smt.executeQuery();
            while (rs.next()) {
                int idEmpleado = rs.getInt("id_cliente");
                String nombre = rs.getString("nombre");
